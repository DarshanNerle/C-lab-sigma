...
async function generateWithAnthropic({ apiKey, systemPrompt, levelInstruction, modeInstruction, level, topic, formattedHistory, message }) {
  const anthropic = createAnthropic({ apiKey });
  const prompt = [
    `Explain Mode: ${levelInstruction}`,
    `Instructions: ${modeInstruction}`,
    `Context: Student is at ${level || 'High School'} level, studying ${topic || 'General Chemistry'}.`,
    ...formattedHistory.map((item) => `${item.role === 'assistant' ? 'Assistant' : 'User'}: ${item.content}`),
    `User: ${message.trim()}`
  ].join('\n\n');
